...
CREATE TABLE public.voice_datas (
    id uuid DEFAULT public.gen_random_uuid() NOT NULL,
    "fileUrl" text NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
...
ALTER TABLE ONLY public.voice_datas
...
CREATE TRIGGER set_public_voice_datas_updated_at BEFORE UPDATE ON public.voice_datas FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
COMMENT ON TRIGGER set_public_voice_datas_updated_at ON public.voice_datas IS 'trigger to set value of column "updated_at" to current timestamp on row update';
ALTER TABLE ONLY public.comments
